...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("¿ Quiere guardar el registro ?")){
						e.preventDefault();
						data = new FormData($('#currency-form')[0]);//form name
						/*Check XSS Code*/
						...
						$.ajax({
						type: 'POST',
						url: base_url+'currency/newcurrency',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             // alert(result);return;
							if(result=="success")
							{
								...
								window.location=base_url+"currency/view";
								return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("¿ Desea actualizar el registro ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'currency/update_currency',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
//Delete Record start
function delete_currency(q_id)
{
	
   if(confirm("¿ Seguro que quiere eliminar el registro ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_currency",{q_id:q_id},function(result){
   //alert(result);return;
	   if(result=="success")
				{
