...
if ( ! function_exists('send_sms_using_template'))
{
    function send_sms_using_template($data_id,$template_id){
    	...
    	*/
    	$CI =& get_instance();
		$q1=$CI->db->query("select * from db_smstemplates where id='$template_id'");
		if($q1->num_rows()>0){
			...
			if(!empty($content)){
				switch ($template_id) {
					case 1:
						...
						if($q2->num_rows()>0 && $q2->row()->customer_id!=1){
							...
							*/
							...
							$content = str_replace("{{sales_amount}}", $CI->currency(number_format($q2->row()->grand_total,2,'.','')), $content);
							$content = str_replace("{{paid_amt}}", $CI->currency(number_format($q2->row()->paid_amount,2,'.','')), $content);
							$content = str_replace("{{due_amt}}",$CI->currency(number_format($q2->row()->grand_total-$q2->row()->paid_amount,2,'.','')), $content);
