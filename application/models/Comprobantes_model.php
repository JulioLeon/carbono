...
class Comprobantes_model extends CI_Model { 
    ...
    public function load_comprobantes()
	{
        ...
        $opc = 1;
        $query = $this->db->query(" CALL SP_SUCURSAL2($opc,'','','','',@outcomprobantes); ");
    ...
    public function load_tienda()
    {
        $opc = 5;
        $estado = "1";
        $query = $this->db->query(" CALL SP_SUCURSAL2($opc, '','','','".$estado."',@outcomprobantes);");
        return $query->result();
    }
    ...
    {
        $opc = 2;        
        $this->db->query(" CALL SP_SUCURSAL2($opc, '".$codigo."','".$nombre."','".$tienda."','".$estado."',@outcomprobantes);");
