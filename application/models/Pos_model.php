...
class Pos_model extends CI_Model {
	      ...
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
	        	...
	        	if($res2->stock <1){
	        		$str="zero_stock()";
	        		$disabled='';
	        		$bg_color="background-color:#858586";
	        	}
	        	else{
	        		$str="addrow($res2->id)";
	        		$disabled="disabled=disabled";
	        		$bg_color="background-color:#3bb45a";
	        	}
	      ...
	      ?>
	        ...
	        <div class="col-md-3 col-xs-6 " id="item_parent_<?php echo $i;?>" <?php echo $disabled; ?> title='<?php echo $res2->item_name;?>'>
	          <div class="box box-default item_box" id='div_<?php echo $res2->id;?>' onclick="<?php echo $str; ?>"
	           	...
	           	<span class="label label-danger push-right" title="<?php echo $res2->stock; ?> Quantity's in Stock"><?php echo $res2->stock; ?></span>
	            <div class="box-body box-profile">
	            	<!-- <img class="profile-user-img img-responsive img-circle item_image"  src="<?=$img_src;?>" alt="Item picture"> -->
	              <img class=" img-responsive item_image" style="border: 1px solid gray;"  src="<?=$img_src;?>" alt="Item picture">
	              <lable class="text-center search_item" id="item_<?php echo $i;?>"><?php echo substr($res2->item_name,0,25);?></label>
	            </div>
	          </div>
	        </div>
	...
	public function update_items_quantity($item_id){
		...
		$stock=($stock_qty+$pu_tot_qty)-$sl_tot_qty;
		...
		extract($_POST);
	      ...
	      if($q2->num_rows()>0){
	        foreach($q2->result() as $res2){
	      	...
	      	$str =$str."<tr>";
	      		$str =$str.'<td colspan="4" class="text-danger text-center">No se encontraron registros</td>';
	      	$str =$str.'</tr>';
