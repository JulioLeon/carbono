...
class Tax_model extends CI_Model {
	...
	var $table = 'db_tax';
	var $column_order = array('id','tax_name','tax','status'); //set column field database for datatable orderable
	var $column_search = array('id','tax_name','tax','status'); //set column field database for datatable searchable 
	...
	public function verify_and_save(){
		...
		$query=$this->db->query("select * from db_tax where tax=$tax");
		if($query->num_rows()>0){
			return "Tax Percentage already Exist.";
	...
	public function update_tax(){
		...
		//Validate This tax already exist or not
		$query=$this->db->query("select * from db_tax where upper(tax)=upper('$tax') and id<>$q_id");
		if($query->num_rows()>0){
			return "tax Percentage already Exist.";
	...
	public function delete_tax_from_table($ids){	
		$query1="delete from db_tax where id in($ids)";
        if ($this->db->simple_query($query1)){
            echo "success";
        }
        else{
